...
export function ExportPreviewModal({ isOpen, onClose, data }: ExportPreviewModalProps) {
                        ...
                        {viewMode === 'table' ? (
                            <table className="w-full text-sm text-left">
                                <thead className="bg-gray-50/50 border-b border-gray-100 text-gray-500 text-[10px] uppercase font-bold tracking-widest sticky top-0 z-10">
                                    <tr>
                                        <th className="px-4 py-3 font-bold whitespace-nowrap">Item Name</th>
                                        <th className="px-4 py-3 font-bold whitespace-nowrap">Variant</th>
                                        <th className="px-4 py-3 text-right font-bold whitespace-nowrap">Quantity</th>
                                        <th className="px-4 py-3 font-bold whitespace-nowrap">Unit</th>
                                    </tr>
                                </thead>
                                    ...
                                    {exportData.length === 0 ? (
                                        ...
                                        exportData.map((row, idx) => (
                                            <tr key={idx} className="hover:bg-gray-50/50 transition-colors">
                                                <td className="px-4 py-3 font-medium text-gray-900">{row['Item Name']}</td>
                                                <td className="px-4 py-3 text-gray-700">{row['Variant']}</td>
                                                <td className="px-4 py-3 text-right font-mono font-medium">{row['Quantity'].toLocaleString()}</td>
